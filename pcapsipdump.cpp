...
uint32_t hsaddr(void *p)
{
    ...
    else
    {
        ...
        return (uint32_t)((uint32_t)(header_ipv6->saddr[0]) +
...
uint32_t hdaddr(void *p)
{
    ...
    {
        // Ad-hoc 128bit -> 32bit hash function
        return (uint32_t)((uint32_t)(header_ipv6->daddr[0]) +
...
bool get_sip_response(const char *data, char *sip_response, size_t sip_response_size) {
    ...
    // Copy the response into the sip_response buffer
    size_t response_length = MIN((size_t)(response_end - response_start), sip_response_size - 1);
    memcpy(sip_response, response_start, response_length);
    sip_response[response_length] = '\0'; // Null-terminate the response
...
int main(int argc, char *argv[])
{
    ...
    const char *number_filter_pattern = NULL;
    ...
    const char *method_filter_pattern = NULL;
    ...
    if ((res = opts_sanity_check_d(&opt_fntemplate)) != 0)
        return res;
    ct = new calltable;
    ...
    if (ifname)
    {
        ...
        if (method_filter_pattern != NULL) {
            printf("Filtering on SIP Method: '%s'\n", method_filter_pattern);
        }
        
        if (number_filter_in_use) {
            printf("Filtering on Number: '%s'\n", number_filter_pattern);
        }

        if (ip_filter_in_use) {
            printf("Filtering on IP address: '%s'\n", opt_ipfilter);
        }

        if (header_filter_in_use) {
            printf("Filtering on SIP Header: '%s'\n", opt_sip_header_filter);
        }
    ...
    {
        {
            ...
            { // fragment offset > 0
                ...
                if (f)
                {
                    ...
                    if ((header_ip->frag_off & htons(0x2000)) == 0)
                    { // more_fragments == 0
            ...
            )
            {
                ...
                if (frag->nexthdr == IPPROTO_UDP)
                {
                    ...
                    if (f)
                    {
                     ...
                     (header_ip->version == 4 && pkt_header->caplen >= (offset_to_ip + sizeof(struct iphdr) + sizeof(struct udphdr)) &&
                      header_ip->protocol == IPPROTO_UDP)
                     /* sane IPv6 UDP */
                     ||
                     (header_ipv6->version == 6 && pkt_header->caplen >= (offset_to_ip + sizeof(struct ipv6hdr) + sizeof(struct udphdr)) &&
                     ...
                     /* sane IPv4 TCP */
                     ||
                     (header_ip->version == 4 && pkt_header->caplen >= (offset_to_ip + sizeof(struct iphdr) + sizeof(struct tcphdr)) &&
                      header_ip->protocol == IPPROTO_TCP)
                     /* sane IPv6 TCP */
                     ||
                     (header_ipv6->version == 6 && pkt_header->caplen >= (offset_to_ip + sizeof(struct ipv6hdr) + sizeof(struct tcphdr)) &&
                      header_ip->protocol == IPPROTO_TCP)
#endif
            )
            {
                         ...
                         !memcmp(data, "SIP/2.0 ", strlen("SIP/2.0 ")))
                {
                    ...
                    if (verbosity >= 3)
                    {
                        ...
                        // Get response code if not a request
                        if (sip_method[0] == '\0')
                        {
                            char sip_response[256] = "";
                            if (get_sip_response(data, sip_response, sizeof(sip_response)))
                            {
                                printf("SIP Packet [Response]: %s\n", sip_response);
                            }
                        } else {
                            // Print SIP method and details
                            printf("SIP Packet [Request]: %s\n", sip_method);
                        }
                    ...
                    else
                    {
                        ...
                        unsigned long matched_header_len;
                        if (gettag(data, datalen, opt_sip_header_filter, &matched_header_len) != NULL)
                        {
                            if (verbosity >= 2)
                            {
                            ...
                            header_filter_matched = true;
                        } else if (regexec(&method_filter, sip_method, 1, pmatch, 0) == 0)
                        {
                            if (verbosity >= 3)
                            {
...
//                        && header_filter_matched
                    ...
                    // Check if the call is already in the call table
                    if (idx >= 0)
                    {
                        // Set flag when BYE is received
                        ...
                        if (ct->table[idx].f_pcap != NULL)
                        {
                            ...
                            // Flush the dump file if packet buffered mode is enabled
                            if (opt_packetbuffered)
                            {
